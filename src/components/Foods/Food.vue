<template>
  <q-card class="card">
    <q-img :src="food.imageUrl ? food.imageUrl : 'statics/image-placeholder.png'" basic contain>
      <div class="absolute-bottom text-h6">{{ food.name }}</div>
    </q-img>

    <q-card-section>
      <q-rating :value="food.rating" size="2em" color="orange" readonly class="q-mt-sm"/>
    </q-card-section>
    ...
    <q-card-actions class="absolute-bottom" align="right">
      <q-btn @click="showEditFoodModal = true" icon="edit" color="blue" flat>Edit</q-btn>
      <q-btn @click="promptToDelete" icon="delete" color="red" flat>Delete</q-btn>
    </q-card-actions>
...
export default {
  ...
  components: {
    'modal-add-edit-food': require('components/Foods/Modals/ModalAddEditFood.vue')
      .default
  }
}
</script>
